...
#include "unitree_gazebo/unitree_gazebo_system.hpp"
...
bool UnitreeGazeboSystem::initSim(
  rclcpp::Node::SharedPtr & model_nh,
  gazebo::physics::ModelPtr parent_model,
  const hardware_interface::HardwareInfo & hardware_info,
  sdf::ElementPtr sdf)
{
  this->dataPtr = std::make_unique<UnitreeGazeboSystemPrivate>();
  ...
  gazebo::physics::ModelPtr parent_model)
{
  ...
  for (unsigned int j = 0; j < this->dataPtr->n_dof_; j++) {
    ...
    for (unsigned int i = 0; i < joint_info.command_interfaces.size(); i++) {
      ...
      if (joint_info.command_interfaces[i].name == "k_p") {
        RCLCPP_INFO_STREAM(this->nh_->get_logger(), "\t\t k_p");
        this->dataPtr->command_interfaces_.emplace_back(
          joint_name + suffix,
      ...
      if (joint_info.command_interfaces[i].name == "k_d") {
        RCLCPP_INFO_STREAM(this->nh_->get_logger(), "\t\t k_d");
        this->dataPtr->command_interfaces_.emplace_back(
          joint_name + suffix,
