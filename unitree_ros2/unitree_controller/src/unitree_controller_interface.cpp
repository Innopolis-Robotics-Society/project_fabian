    ...
    const rclcpp::Time & time, const rclcpp::Duration & period) 
{
  ...
  {
    states_.q_j.coeffRef(i)   = qJ_interface_[i].get().get_value();
    states_.dqJ.coeffRef(i)  = dqJ_interface_[i].get().get_value();
    states_.tau_j.coeffRef(i) = tauJ_interface_[i].get().get_value();
  }
  // get Imu state
